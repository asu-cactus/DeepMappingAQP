...
class DataGen:
    ...
    def __init__(self):
        ...
        parser.add_option(
            ...
            action="store",
            type=int,
            help="Number of samples to draw from the original dataset",
            default=5000000,
        )
        parser.add_option(
            "-o",
    ...
    def normalize(self):
        ...
        for chunk_id, chunk in enumerate(
            pd.read_csv(self.options.normalize, chunksize=100000, header=0)
        ):
            all_from_fields = []
            for tbl in tables:
                ...
                for mapping in tbl["mapping"]:
                    ...
                    chunk = pd.merge(
                        ...
                        table_df,
                        how="left",
                        left_on=mapping["fromFields"],
                        right_on=tbl["columns"],
                    )
    ...
    def generate_data(self):
        ...
        self.derived_cols = [
            col
            for col in self.sample_json["tables"]["fact"]["fields"]
            if "deriveFrom" in col
        ]
        self.derivates = {}
        for derived_col in self.derived_cols:
            kk = (
                self.df.groupby(derived_col["deriveFrom"])[derived_col["field"]]
                .first()
                .to_dict()
            )
        ...
        for cat_col in self.cat_cols:
            self.cat_hists[cat_col] = (
                self.df[cat_col].value_counts(normalize=True).to_dict()
            )
            self.cat_hists[cat_col] = OrderedDict(
                sorted(self.cat_hists[cat_col].items(), key=lambda x: x[0])
            )
    ...
    def process_batch(self, batch_number):
        ...
        if (batch_number + 1) * self.options.batchsize > self.options.size:
            num_samples_to_generate = (
                self.options.size - (batch_number) * self.options.batchsize
            )
        else:
        ...
        for derived_col in self.derived_cols:
            new_col = result_frame[derived_col["deriveFrom"]].map(
                self.derivates[derived_col["field"]]
            )
        ...
        if batch_number == 0:
            ...
            result_frame.to_csv(
                self.options.output, index=False, header=False, mode="a"
            )
